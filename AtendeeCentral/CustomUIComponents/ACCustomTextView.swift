//
//  ACCustomTextView.swift
//  AtendeeCentral
//
//  Created by PROBIR CHAKRABORTY on 22/08/16.
...
class ACCustomTextView: UITextView {
    // variables
    ...
    @IBInspectable var placeholderColor: UIColor = KAppPlaceholderColor
    ...
    func listenForTextChangedNotifications() {
        NSNotificationCenter.defaultCenter().addObserver(self, selector: #selector(ACCustomTextView.textChangedForPlaceholderTextView(_:)), name:UITextViewTextDidChangeNotification , object: self)
        NSNotificationCenter.defaultCenter().addObserver(self, selector: #selector(ACCustomTextView.textChangedForPlaceholderTextView(_:)), name:UITextViewTextDidBeginEditingNotification , object: self)
    }
