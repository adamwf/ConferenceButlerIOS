...
class CVCalendarManager {
    ...
    func weeksWithWeekdaysForMonthDate(date: NSDate) -> (weeksIn: [[Int : [Int]]], weeksOut: [[Int : [Int]]]) {
        ...
        let countOfDaysIn = CalenderManager.dateRange(lastMonthDateIn).day
        let countOfDaysOut = totalCountOfDays - countOfDaysIn
        
        // Find all dates in.
        var datesIn = [NSDate]()
        for day in 1...countOfDaysIn {
            let components = CalenderManager.componentsForDate(firstMonthDateIn)
            components.day = day
        ...
        let components = CalenderManager.componentsForDate(firstWeekDate)
        components.day += 6
        var lastWeekDate = calendar.dateFromComponents(components)!
        
        func nextWeekDateFromDate(date: NSDate) -> NSDate {
            let components = CalenderManager.componentsForDate(date)
            components.day += 7
        ...
        for i in 0..<count {
            ...
            for weekday in 1...7 {
                ...
                components.day += 1
                let day = CalenderManager.dateRange(weekdate).day
    ...
    class func dateFromYear(year: Int, month: Int, week: Int, day: Int) -> NSDate? {
        let comps = CalenderManager.componentsForDate(NSDate())
        comps.year = year
        comps.month = month
